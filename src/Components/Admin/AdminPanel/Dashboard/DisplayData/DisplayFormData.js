...
const DisplayData = (props) => {
    ...
    const deleteForm = () => {
        dispatch(removeFromFirebase(`https://study-49f96-default-rtdb.europe-west1.firebasedatabase.app/forms/${props.data.key}.json`));
        props.clicked();
      };
    return (
        <div className={classes.DisplayData}>
            ...
            <div className={classes.Left}>
                <p><strong>Imię:</strong> {props.data.name}</p>
                <p><strong>Nazwisko:</strong> {props.data.surname}</p>
                <p><strong>Adres e-mail:</strong> {props.data.email}</p>
                <p><strong>Telefon:</strong> {props.data.tel}</p>
                <p><strong>Data ślubu:</strong> {props.data.date}</p>
                <p><strong>Miejsce ślubu:</strong> {props.data.church}</p>
                <p><strong>Miejsce wesela:</strong> {props.data.weddinPlace}</p>
                <p><strong>Planowana liczba gości:</strong> {props.data.guests}</p>
                <p><strong>Kształt stołów:</strong> {props.data.table}</p>
                <p><strong>Budżet:</strong> {props.data.budget}</p>
