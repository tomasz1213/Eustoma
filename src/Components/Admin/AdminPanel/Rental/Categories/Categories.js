...
const Categories = (props) => {
	const [categoryName, setCategoryName] = useState(
		props.editMode ? props.data.name : ""
	);
	...
	const uploader = () => {
		...
		if (!props.editMode) {
			dispatch(
				uploadImage("input_categories", `/categories.json?auth=${auth}`, {
					name: categoryName,
				})
			);
			props.clicked();
			...
			dispatch(
				removeFromFirebase(`/categories/${props.data.key}.json?auth=${auth}`)
			);
			props.clicked();
		}
	};
	...
	return (
		<div className={classes.InputLabel}>
			...
			<label>
				...
				<button
					style={{ top: "-10px" }}
					onClick={() => document.getElementById("input_categories").click()}
					className={classes.Button}
				>
					Dodaj Zdjęcie
				</button>
				{props.editMode && (
					<img className={classes.Image} alt="foto" src={props.data.url}></img>
				)}
				...
				<input
					...
					type="text"
					id="input_categories--text"
				></input>
				<p
					style={{ color: "red", fontSize: "14px", padding: "2px" }}
					id="CATEGORIES_ERRORP"
				></p>
				<button
					style={{ top: "10px" }}
					className={classes.Button}
