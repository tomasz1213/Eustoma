...
const Products = (props) => {
	...
	const uploader = () => {
		...
		if (!props.editMode) {
			dispatch(
				uploadImage("input_products--photo", `/products.json?auth=${auth}`, {
					...inputData,
	...
	const deleteProduct = () => {
		...
		dispatch(
			removeFromFirebase(`/products/${props.data.key}.json?auth=${auth}`)
		);
		props.clicked();
	};
	...
	return (
		<div className={classes.InputLabel}>
			...
			<input
				...
				type="file"
				id="input_products--photo"
				multiple
			></input>
			<button
				style={{ top: "-10px" }}
				onClick={() => document.getElementById("input_products--photo").click()}
			...
			<textarea
				...
				onChange={(event) => handleInputs(event.target)}
				type="text"
				id="input_products--description"
			></textarea>
			<p style={{ color: "red", fontSize: "14px", padding: "2px" }}></p>
			<p>WYBIERZ KATEGORIĘ PRODUKTU:</p>
			<select
				value={inputData.itemCategory}
				className={classes.Input}
				onChange={(event) => handleInputs(event.target)}
				name="input_products--itemCategory"
				id="input_products--itemCategory"
			>
			...
			<input
				...
				onChange={(event) => handleInputs(event.target)}
				type="text"
				id="input_products--items"
			></input>
			<p
				style={{ color: "red", fontSize: "14px", padding: "2px" }}
				id="PRODUCT_ERROR"
			></p>
			<p>DODAJ CENĘ CZYSZCZENIA:</p>
			<input
				className={classes.Input}
				value={inputData.cleanPrize}
				onChange={(event) => handleInputs(event.target)}
				type="text"
				id="input_products--cleanPrize"
			></input>
			<p
				style={{ color: "red", fontSize: "14px", padding: "2px" }}
				id="PRODUCT_ERROR"
			...
			<input
				className={classes.Input}
				value={inputData.prize}
				onChange={(event) => handleInputs(event.target)}
				type="text"
				id="input_products--prize"
			></input>
			<p
				style={{ color: "red", fontSize: "14px", padding: "2px" }}
				id="PRODUCT_ERROR"
