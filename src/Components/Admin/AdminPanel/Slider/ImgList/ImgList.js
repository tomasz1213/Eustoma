...
const ImgList = (props) => {
	...
	const uploader = () => {
		dispatch(
			uploadImage(
				"input_slider",
				`https://study-49f96-default-rtdb.europe-west1.firebasedatabase.app/sliders/${props.name}.json?auth=${auth}`
			)
		);
	};
	const deleteElement = (elementId) => {
		...
		if (answer) {
			...
			dispatch(
				removeFromFirebase(
					`https://study-49f96-default-rtdb.europe-west1.firebasedatabase.app/sliders/${props.name}/${elementId}.json?auth=${auth}`,
					elementId
				)
			);
		} else {
	...
	const displayImgList = () => {
		if (data.length > 1) {
			...
			element = dataArray.map((el) => (
				<Element
					clicked={() => deleteElement(el.key)}
					key={el.key}
					name={el.value.name}
					background={el.value.url}
				/>
			));
		}
	};
