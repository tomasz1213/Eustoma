...
const RentalMain = (props) => {
    ...
    const sortByCategory = (category) => {
        setTimeout(() => {
            const categoriesList = [...document.getElementsByClassName(`${classes.CategoriesLI}`)];
            const newCategory = products.filter(el => el.itemCategory === category);
            const activeButton = categoriesList.find(el => el.outerText === category);

            categoriesList.forEach(category => category.className = classes.CategoriesLI);
    ...
    const sortProductsBy = (type) => {
        switch(type){
            case 'fresh':{
                const sortedArr = [...sortedProducts].sort((a,b) =>new Date(b.date) - new Date(a.date));
                setSortedProducts(sortedArr);
                break;
            }
            case 'nameA':{
                const sortedArr = [...sortedProducts].sort((a,b) => a.name.localeCompare(b.name));
                setSortedProducts(sortedArr);
                break;
            }
            case 'nameZ':{
                const sortedArr = [...sortedProducts].sort((a,b) => a.name.localeCompare(b.name)).reverse();
                setSortedProducts(sortedArr);
                break;
            }
            case 'prize-':{
                const sortedArr = [...sortedProducts].sort((a,b) => Number(a.prize)-Number(b.prize));
                setSortedProducts(sortedArr);
                break;
            }
            case 'prize+':{
                const sortedArr = [...sortedProducts].sort((a,b) => Number(a.prize)-Number(b.prize)).reverse();
                setSortedProducts(sortedArr);
                break;
            }
            default:
        ...
        <>
            ...
            :<div className={classes.Conteiner}>
                <div className={classes.CatMobile}>
                    <span style={{fontSize:'24px',borderBottom: '1px solid #dee0e6',paddingBottom: '10px'}}>WYPOŻYCZALNIA</span>
                    <p style={{fontSize:'16px',paddingTop: '10px'}}>KATEGORIE</p>
                    <ul className={classes.CategoriesList}>
                        <li className={`${classes.Active} ${classes.CategoriesLI}`} onClick={()=>sortByCategory('Wszystkie')}>Wszystkie</li>
                        {categories.map(el => <li className={classes.CategoriesLI} onClick={()=>sortByCategory(el.name)} key={el.name + el.name}>{el.name}</li>)}
                    </ul>
                    <div style={{width:'213.39px',height:'1px',borderBottom: '1px solid #dee0e6',marginLeft:'auto',marginRight:'auto'}}></div>
                </div>
                <div className={classes.Products}>
                    <select onChange={(event)=> sortProductsBy(event.target.value)} className={classes.Sorting}>
