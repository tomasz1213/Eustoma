...
const RentalSlider = (props) => {
	...
	const showPhotoElements = productData.map((element) => (
		<NavLink key={element.key} to="/rental/product">
			<Product
				type={false}
				fresh={computeFresh(element.date)}
	...
	const moveSlider = (side) => {
		...
		if (SLIDER_COUNTER >= countMoves && side === true) {
			// left button
	...
	return (
		<>
			<span onClick={() => moveSlider(false)} className={classes.SliderLeft}>
				<i className="icon-left-open-big"></i>{" "}
			</span>
			<div className={classes.Slider}>
				<div className={classes.Foto}>{showPhotoElements}</div>
			</div>
			<span onClick={() => moveSlider(true)} className={classes.SliderRight}>
