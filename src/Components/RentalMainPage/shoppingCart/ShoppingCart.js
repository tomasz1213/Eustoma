...
const ShoppingCart = (props) => {
	...
	const [modalDataHandler, setModalDataHandler] = useState({
		date: null,
		location: null,
		name: null,
		email: null,
	...
	const submitShoppingCart = () => {
		if (
			modalDataHandler.date &&
			modalDataHandler.location &&
			modalDataHandler.phone
		) {
			dispatch(
				uploadDataFirebase("/shoppingCart.json", {
					...modalDataHandler,
					cart: [...shoppingCar.cart],
				})
			);
	...
	const handleInputs = (event, type) => {
		if (event.target.value.length < 1)
			event.target.style.backgroundColor = "#f08b8b";
		else {
		...
		<>
			{!displayShoppingCart && (
				...
				>
					...
					<div className={classes.CartNumberConteiner}>
						<div className={classes.CartNumber}>{shoppingCar.value}</div>
					</div>
				</div>
			)}
			{displayShoppingCart && (
				<div className={classes.ShoppingCartResult}></div>
			)}
			{displayModal && (
				<div className={classes.Modal}>
					...
					<input
						onChange={(event) => handleInputs(event, "location")}
						type="text"
						className={classes.ModalInput}
					></input>
					...
					></input>
					{displayError && (
						<div style={{ color: "red" }}>Proszę uzupełnić wszystkie pola.</div>
					)}
					<div onClick={submitShoppingCart} className={classes.ModalSubmit}>
						Wyślij
					</div>
				</div>
			)}
			{displayShoppingCart && (
				<aside className={classes.SideBar}>
					...
					<div className={classes.Products}>
						{shoppingCar.cart &&
							shoppingCar.cart.map((el) => {
								return (
									<div key={el.key} className={classes.Product}>
										<div className={classes.ProductImage}>
											<img src={el.url} alt={el.name} />
										</div>
										<div className={classes.ProductInfo}>
											<span className={classes.ProductName}>{el.name}</span>
											<span className={classes.ProductPrize}>
												zł {el.prize}
											</span>
											<input
												className={classes.ProductInput}
												onChange={(event) =>
													addToShoppingCart(event.target.value, el)
												}
												type="number"
												value={el.productAmount}
											></input>
											<div
												className={classes.ProductDelete}
												onClick={() => removeFromShopppingCart(el.key)}
											>
												X
											</div>
										</div>
					...
					<div className={classes.Bottom}>
						<div className={classes.Prize}>
							Kwota Częściowa:{" "}
							<p style={{ margin: "5px", fontSize: "23px" }}>
								{shoppingCar.totalPrize} zł
							</p>
							<div
