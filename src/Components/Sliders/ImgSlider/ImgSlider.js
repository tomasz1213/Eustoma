...
const ImgSlider = () => {
	...
	const moveSlider = (side) => {
		// Ture for left False for right
		const idSlider = document.getElementById("SLIDER_IMG");
		const sliderClass = [
			...document.getElementsByClassName(`${idSlider.className}`),
		];
		const imgStyle = window.getComputedStyle(sliderClass[0]);
		const elementParent = window.getComputedStyle(sliderClass[0].parentNode);
		const elementParentWidth = elementParent.width.slice(
			1,
			elementParent.width.length - 2
		);
		const elementCountedAmount =
			260 * (sliderClass.length - 3) - Number(elementParentWidth);
		...
		intervalSlider = setInterval(() => {
			...
			if (
				Number(imgStyle.left.slice(1, imgStyle.left.length - 2)).toFixed(0) >
				Number(Math.ceil(elementCountedAmount / 10) * 10)
			) {
				clearInterval(intervalSlider);
				sliderClass.forEach((elm) => (elm.style.left = `${nextValue + 200}px`));
	...
	const showImageElements = images.map((e) => (
		<ImgElement
			type={true}
			key={images.indexOf(e)}
			src={e}
			alt="Slider"
			nextImg={{
				arr: images,
	...
	return (
		<div className={classes.Slider}>
			<span
				onMouseOver={() => moveSlider(false)}
				onMouseOut={() => clearInterval(intervalSlider)}
				className={classes.SliderLeft}
			>
				<i className="icon-left-open-big"></i>{" "}
			</span>
			<div className={classes.Foto}>{showImageElements}</div>
			<span
				onMouseOver={() => moveSlider(true)}
				onMouseOut={() => clearInterval(intervalSlider)}
