...
const Tile = (props) => {
  ...
  return (
    <div
      onClick={() => history.push(props.ahref)}
      onMouseOver={(event) => handleElementHover(event, 0)}
      onMouseOut={(event) => handleElementHover(event, "-30px")}
      className={classes.Tile}
      style={{
        backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.327),rgba(0, 0, 0, 0.3)) ,url(${props.background})`,
