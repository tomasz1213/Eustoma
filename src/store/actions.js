...
export const updateDataFirebase = (nodeName, id, data, inputID) => {
	// nodeName = firebase Node Name
	return async (dispatch) => {
		const combineArr = [];
		dispatch(changeLoadingToTrue());
		const userRef = database.ref(nodeName).child(id);
		await uploadMultipleImages(inputID).then(
			(arr) => (combineArr = [...arr, ...data.url])
		);
...
const uploadMultipleImages = (input) => {
	// func for assigning more img to 1 db element
	return new Promise(async (resolve, reject) => {
		...
		for (let i = 0; i < file.files.length; i++) {
			const compressedPhoto =
				file.files[i].size > 1700000
					? await handleCompressedUpload(file.files[i])
					: file.files[i];
			const fileRef = storageRef.child("images/" + file.files[i].name);
			const uploadTaskSnapshot = await fileRef.put(
				file.files[i].size > 1700000 ? compressedPhoto : file.files[i]
			);
...
export const uploadImage = (input, url, data) => {
	return async (dispatch) => {
		const isInputFromForm =
			input === "input_form--photo" ? "forms/" : "images/";
		dispatch(changeLoadingToTrue());

		if (
			input === "input_products--photo" ||
			input === "input_ourwork--photo" ||
			input === "input_form--photo"
		) {
			// Those input's have ability to upload more than one image to single DB node
			await uploadMultipleImages(input).then((fotoArr) =>
				dispatch(
					uploadDataFirebase(url, { url: fotoArr, name: data.name, ...data })
				)
			);
		} else {
			const file = document.getElementById(input);
			if (file.files.length === 0)
				dispatch(
					uploadDataFirebase(url, { url: [], name: data.name, ...data })
				);
			for (let i = 0; i < file.files.length; i++) {
				const compressedPhoto = await handleCompressedUpload(file.files[i]);
				const uploadTask = storageRef
					.child(isInputFromForm + file.files[i].name)
	...
	return (dispatch) => {
		...
		axios
			...
			.then(() => {
				...
				if (optionalURL) {
					axios.get(optionalURL).then((res) => {
						for (const [key, value] of Object.entries(res.data)) {
